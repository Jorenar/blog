...
C, C#, COBOL, Fortran, J and Object Pascal. Beside C and C#, those aren't
languages which one would call mainstream nowadays. Only in COBOL, using VLA
can be really considered safe (because of the requirement to specify the maximal size).
</aside>
...
Let's consider a painfully simple, very favourable to VLA, example:
```c
#include <stdio.h>
